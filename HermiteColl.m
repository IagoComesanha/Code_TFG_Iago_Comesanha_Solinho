function [Fi, Gi, Hi, lbounds, ubounds] = HermiteColl(variab, ...
    nv, th, Phi, f, g, c, conx, conf, lboundsp, lboundsu, ...
    uboundsp, uboundsu, lboundsx, uboundsx, lboundsz, uboundsz)
% Función para obter a función obxectivo e restricións do problema 
% NLP mediante un esquema de discretización de Hermite-Simpson.
%
% DATOS DE ENTRADA:
...
% f(x(t_i-1)), g(x(t_i-1)), c(x(t_i-1))
fi = cell(f{end,1}, nv); gi = cell(g{end,1}, nv); 
ci = cell(c{end,1}, nv);
for i = 1:nv
    % Índices respectivos no vector xi para conseguir as variables
    % avaliadas en t_i-1.
    indx = (i0 + (i-1)*nvar + 1):(i0 + (i-1)*nvar + variab.ndif);
    indz = (i0+ (i-1)*nvar + variab.ndif + 1):(i0 + ...
                          (i-1)*nvar + variab.ndif + variab.nalx);
    indu = (i0 + (i-1)*nvar + variab.ndif + ...
                                   variab.nalx + 1):(i0 + i*nvar);
...
% \bar{x}^{k+1}
barx = cell(variab.ndif, nv);
for j = 1:variab.ndif
    barx{j,1} = @(xi) xi(i0 + j);
    for i = 2:nv
        indk = i0 + (i-2)*nvar + j; 
        indk1 = i0 + (i-1)*nvar + j;
        barx{j,i} = @(xi) (1/2)*(xi(indk) + xi(indk1)) + ...
                        (h(i-1)/8)* (fi{j,i-1}(xi) + fi{j,i}(xi));
    end
end

% \bar{z}^{k+1/2};
zmedio = cell(variab.nalx, nv);
for j = 1:variab.ncon
    zmedio{j,1} = @(xi) xi(i0 + j);
    for i = 2:nv
        indk = i0 + (i-2)*nvar + variab.ndif + j; 
        indk1 = i0 + (i-1)*nvar + variab.ndif + j;
        zmedio{j,i} = @(xi) (1/2)*(xi(indk) + xi(indk1));
    end
end

% \bar{u}^{k+1/2}
sumar = variab.ndif + variab.nalx;
umedio = cell(variab.ncon, nv);
for j = 1:variab.ncon
    umedio{j,1} = @(xi) xi(i0 + sumar + j);
    for i = 2:nv
        indk = i0 + (i-2)*nvar + sumar + j; 
        indk1 = i0 + (i-1)*nvar + sumar + j;
        umedio{j,i} = @(xi) (1/2)*(xi(indk) + xi(indk1));
    end
end

% \bar{f}^{k+1}
barf = cell(f{end,1}, nv);
for j = 1:f{end,1}
    barf{j,1} = @(xi) 0;
    for i = 2:nv
        barf{j,i} = @(xi) f{j,1}(barmat(barx,xi,i), ...
              barmat(zmedio,xi,i), barmat(umedio,xi,i), xi(1:i0));
    end
end
...
for j = 1:variab.ndif
    ...
    for i = 2:nv
        ...
        preG{j,i} = @(xi) xi(ind1) - xi(ind) - ...
         h(i-1)*(fi{j,i-1}(xi) + 4.*barf{j,i}(xi) + fi{j,i}(xi))/6; 
    end
end
% Condicións finais.
