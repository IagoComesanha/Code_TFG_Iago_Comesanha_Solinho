...
variab = struct('ndif', 3, 'nalx', 0, 'ncon', 1, 'npar', 0);

% Función obectivo, funcional a minimizar. Definida segundo a
% discretización.
Phi =  @(xh,zh,uh,ph) xh(3,end);
...
f = {@(x,z,u,p) x(2);...
     @(x,z,u,p) -x(1) + (1.4 - 0.14.*(x(2).^2)).*x(2) + 4.*u(1);...
     @(x,z,u,p) x(1).^2 + u(1).^2;...
     3};
...
uboundsx = {Inf; Inf; Inf; 3}; uboundsz={0}; uboundsu = {Inf; 1}; 
uboundsp = {0};
lboundsx = {-Inf; -Inf; -Inf; 3}; lboundsz = {0};
lboundsu = {-Inf; 1}; lboundsp = {0};
